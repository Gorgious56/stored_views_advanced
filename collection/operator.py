...
def iterate_collections(context, store, index, parent, do):
    collections_props = context.scene.sva.get(store).get(index).collections
...
class SVA_OT_collections_store_settings(bpy.types.Operator):
    ...
    def execute(self, context):
        iterate_collections(
            context, self.store, self.index, context.scene.collection, lambda props, col: props.store(col)
        )
        return {"FINISHED"}
...
class SVA_OT_collections_restore_settings(bpy.types.Operator):
    ...
    def execute(self, context):
        iterate_collections(
            context, self.store, self.index, context.scene.collection, lambda props, col: props.restore(col)
        )
